...
if [ "$STATUS" = "Playing" ] || [ "$STATUS" = "Paused" ]; then
    ...
    if [ -f "$finalArt" ] && [ -s "$finalArt" ]; then
        ...
        if [ ! -f "$lockFile" ] && [ -n "$rawUrl" ]; then
            ...
            (
                ...
                if [ ! -s "$finalArt" ]; then cp "$PLACEHOLDER" "$finalArt"; fi
    ...
    if [ -z "$len_micro" ] || [ "$len_micro" -eq 0 ] 2>/dev/null; then len_micro=1000000; fi
    len_sec=$((len_micro / 1000000))
    pos_sec=$((pos_micro / 1000000))
    [ "$len_sec" -le 0 ] 2>/dev/null && len_sec=1
    ...
    sink_name=$(pactl get-default-sink 2>/dev/null | xargs)
    dev_icon="󰓃"; dev_name="Speaker"
    if [[ "$sink_name" == *"bluez"* ]]; then
        dev_icon="󰂯"
        readable_name=$(pactl list sinks 2>/dev/null | grep -A 20 "$sink_name" | grep -m 1 "Description:" | cut -d: -f2 | xargs)
        dev_name="${readable_name:-Bluetooth}"
    elif [[ "$sink_name" == *"usb"* ]]; then
        dev_icon="󰓃"; dev_name="USB Audio"
    elif [[ "$sink_name" == *"pci"* ]]; then
        dev_icon="󰋎"; dev_name="System"
    fi
