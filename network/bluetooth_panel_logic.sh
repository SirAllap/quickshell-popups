...
get_status() {
    ...
    if [ "$power" == "on" ]; then
        paired_macs=$(bluetoothctl devices Paired 2>/dev/null | grep -E "^Device ([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2} " | cut -d ' ' -f 2)
        mapfile -t devices < <(bluetoothctl devices 2>/dev/null | grep -E "^Device ([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2} ")

        connected_list_objs=()
        paired_list_objs=()
        discovered_list_objs=()

        # Get all connected devices (filter out Controller, Agent, and other non-device lines)
        mapfile -t connected_info_lines < <(bluetoothctl devices Connected 2>/dev/null | grep -E "^Device ([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2} ")
